...
func main() {
    ...
    filteredConfigs := filterForProtocols(allConfigs)
    ...
    for _, config := range filteredConfigs {
        for _, protocol := range protocols {
            if strings.HasPrefix(strings.ToLower(config), strings.ToLower(protocol)) {
                protocolCounts[strings.TrimSuffix(protocol, "://")]++
                break
            }
        }
    }
    ...
    fmt.Println("Sorting configs by country...")
    sortConfigs()
    sortByCountry()

    fmt.Println("Sorting configs by protocol...")
    sortByProtocol()

    fmt.Println("Configuration aggregation and sorting completed successfully!")
}
...
func filterForProtocols(data []string) []string {
    var filtered []string
    ...
    for _, line := range data {
        ...
        for _, protocol := range protocols {
            if strings.HasPrefix(strings.ToLower(line), strings.ToLower(protocol)) {
                ...
                if protocol == "vmess://" && !isValidVmessConfig(line) {
                    continue
                }
                if protocol == "vless://" && !isValidVlessConfig(line) {
                    continue
                }
